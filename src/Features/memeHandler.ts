...
export default async (message: Message): Promise<boolean> => {
    ...
    if (message.attachments.size > 0) {
        try {
            ...
            const memeExistente = await Meme.findOne({ messageId: message.id, guildId: message.guild!.id });
            if (!memeExistente) {
              const nuevoMeme = new Meme({
                messageId: message.id, authorId: message.author.id, guildId: message.guild!.id,
                channelId: message.channel.id, points: 0, memeUrl: message.attachments.first()!.url,
              });
