...
const IGNORE_WORDS = ['pero', 'como', 'donde', 'porque', 'cuando', 'estos', 'estamos', 'bueno'];
...
export async function handleCulture(message: Message) {
    ...
    for (const word of words) {
        if (word.startsWith('http') || word.includes('.') || word.length < 5 || IGNORE_WORDS.includes(word)) continue;
