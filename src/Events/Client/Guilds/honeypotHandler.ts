...
export default {
    ...
    async execute(message: Message) {
        ...
        if (settings.honeypotChannel && message.channel.id === settings.honeypotChannel) {
            try {
                ...
                await message.guild.members.ban(message.author.id, {
                    reason: "Hoshiko Security: Activación de Honeypot (Bot de Raid detectado)."
                });
