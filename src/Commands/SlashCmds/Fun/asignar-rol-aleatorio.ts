...
export default {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => 
            sub.setName('crear')
                ...
                .addStringOption(opt => opt.setName('emoji').setDescription('El emoji a detectar (ponlo aquí)').setRequired(true))
                .addIntegerOption(opt => opt.setName('meta').setDescription('Cuantas reacciones se necesitan').setRequired(true))
                .addRoleOption(opt => opt.setName('premio').setDescription('Rol para el autor del mensaje').setRequired(true))
                .addIntegerOption(opt => opt.setName('tiempo').setDescription('Minutos de duración (0 = permanente)').setRequired(true))
        )
        .addSubcommand(sub => 
            sub.setName('lista')
                .setDescription('Ver reglas activas')
        )
        .addSubcommand(sub => 
            sub.setName('borrar')
                .setDescription('Borra una regla')
                .addStringOption(opt => opt.setName('emoji').setDescription('El emoji de la regla a borrar').setRequired(true))
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'lista') {
            ...
            if (!rules.length) return interaction.reply({ content: 'No hay reglas configuradas.', ephemeral: true });

            const embed = new EmbedBuilder().setTitle('🌟 Reglas de Popularidad').setColor('Purple');
            const description = rules.map(r => {
        ...
        if (sub === 'borrar') {
            ...
            return interaction.reply({ content: `🗑️ Regla para ${rawEmoji} eliminada.` });
        }
    }
};