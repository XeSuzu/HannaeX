...
const STATUS_CONFIG = {
    ...
    offline: { color: 0x747F8D, emoji: '⚫', name: 'Desconectado', gradient: '🖤' },
    streaming: { color: 0x593695, emoji: '🟣', name: 'En directo', gradient: '💜' }
} as const;
...
function getUserBadges(member: GuildMember | null): string[] {
    ...
    if (member.permissions.has('Administrator')) badges.push('👑 **Administrador**');
    if (member.permissions.has('ManageGuild')) badges.push('⚙️ **Gestor del Servidor**');
    if (member.permissions.has('ModerateMembers')) badges.push('🛡️ **Moderador**');
    if (member.permissions.has('ManageChannels')) badges.push('📝 **Gestor de Canales**');
...
function getMembershipInfo(member: GuildMember | null): string {
    ...
    const days = Math.floor((Date.now() - member.joinedTimestamp) / (1000 * 60 * 60 * 24));
...
function createPermissionsEmbed(member: GuildMember, targetUser: any): EmbedBuilder {
    ...
    if (perms.has('ManageEmojisAndStickers')) generalPerms.push('😀 Gestionar emojis');
    if (perms.has('ManageEvents')) generalPerms.push('📅 Gestionar eventos');
    if (perms.has('CreateInstantInvite')) generalPerms.push('📨 Crear invitaciones');
    ...
    if (adminPerms.length === 0 && modPerms.length === 0 && generalPerms.length === 0) {
