...
const command: SlashCommand = {
  ...
  async execute(interaction, client): Promise<void> {
    try {
      // 🔒 VALIDACIÓN DE DUEÑO
      const creatorId =
        process.env.BOT_OWNER_ID || client.application?.owner?.id;

      if (interaction.user.id !== creatorId) {
        await interaction.reply({
          content:
            "🌸 Nyaa... por ahora este secreto solo lo puede revelar mi creadora. ¡Vuelve más adelante! ✨",
          ephemeral: true,
        });
      ...
      if (!creatorId) {
        await interaction.editReply({
          content: "😿 No encontré la información de mi creadora...",
        });
        return;
      }
      ...
      if (!creator) {
        await interaction.editReply({
          content: "😿 Nyaa~ No pude cargar los datos...",
        });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setImage(
          "https://i.pinimg.com/1200x/eb/a3/43/eba34334f0141ac5ef1ab5c3819b300d.jpg",
        );
      ...
      if (interaction.deferred || interaction.replied) {
        await interaction
          .editReply({ content: "😿 Algo salió mal... inténtalo de nuevo." })
          .catch(() => {});
      } else {
        await interaction
          .reply({ content: "😿 Algo salió mal...", ephemeral: true })
          .catch(() => {});
      }
    }
  },
