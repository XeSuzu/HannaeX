...
const command: SlashCommand = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("view")
        .setDescription("Muestra la configuración actual de strikes."),
    )
    .addSubcommand((sub) =>
      sub
        .setName("add")
        ...
        .addIntegerOption((opt) =>
          opt
            .setName("points")
            .setDescription("Puntos necesarios.")
            .setRequired(true),
        )
        .addStringOption((opt) =>
          opt
        ...
        .addStringOption((opt) =>
          opt
            .setName("duration")
            .setDescription("Duración (ej: 10m).")
            .setRequired(false),
        ),
    )
    .addSubcommand((sub) =>
      sub
        ...
        .addIntegerOption((opt) =>
          opt
            .setName("points")
            .setDescription("Puntos de la regla.")
            .setRequired(true),
        ),
    )
    .addSubcommand((sub) =>
      ...
      switch (subcommand) {
        case "view": {
          const embed = new EmbedBuilder()
            ...
            .setColor(0xeba6d1);
          const logChannel = config.modLogChannel
        ...
        case "add": {
          ...
          if (action === "timeout") {
            if (!durationStr)
              return interaction.editReply({
                content: "❌ Falta duración para timeout.",
              });
            const parsed = parseDuration(durationStr);
            if (!parsed)
              return interaction.editReply({ content: "❌ Formato inválido." });
            durationMs = parsed;
          }

          if (config.autoActions.some((act: any) => act.points === points)) {
            return interaction.editReply({
              content: `❌ Ya existe una regla para ${points} puntos.`,
            });
          }
          ...
          await config.save();
          return interaction.editReply({
            content: `✅ Regla añadida para **${points}** puntos.`,
          });
        }
        ...
        case "remove": {
          ...
          config.autoActions = config.autoActions.filter(
            (act: any) => act.points !== points,
          );
        ...
        case "set-log-channel": {
          ...
          await config.save();
          return interaction.editReply({
            content: `✅ Logs configurados en ${channel}.`,
          });
        }
      }
    } catch (error) {
