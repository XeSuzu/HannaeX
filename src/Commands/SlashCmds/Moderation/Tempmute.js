...
function convertirTiempo(tiempo) {
  ...
  switch (unidad) {
    case "s":
      return cantidad * 1000; // Segundos
    case "m":
      return cantidad * 60 * 1000; // Minutos
    case "h":
      return cantidad * 60 * 60 * 1000; // Horas
    case "d":
      return cantidad * 24 * 60 * 60 * 1000; // Días
    default:
      return null;
  }
}
  ...
  async execute(interaction) {
    ...
    if (
      ...
      return interaction.reply({
        content:
          "❌ No puedo mutear a este usuario por su jerarquía. ¡Es demasiado poderoso!",
    ...
    if (!rolMute) {
      try {
        ...
        return interaction.editReply({
          content:
            "❌ Hubo un error al crear el rol de mute. Revisa los permisos del bot.",
          flags: MessageFlags.Ephemeral,
        });
      }
    }
    ...
    try {
      await miembro.roles.add(
        rolMute,
        `Muteado por ${interaction.user.tag} - Razón: ${razon}`,
      );
      ...
      setTimeout(async () => {
        if (miembro.roles.cache.has(rolMute.id)) {
          ...
          const embedUnmute = new EmbedBuilder()
            ...
            .setDescription(
              `**${usuario.tag}** ahora puede hablar. ¡Qué alegría! 😊`,
            )
            .setTimestamp();
      ...
      return interaction.reply({
        content:
          "❌ Ocurrió un error al mutear. Revisa mi jerarquía de roles o mis permisos. ¡Soy solo un gatito indefenso! 😿",
        flags: MessageFlags.Ephemeral,
      });
    }
  },
