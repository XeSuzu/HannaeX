...
export default {
  ...
  data: new SlashCommandBuilder()
    .setName("warn")
    .setDescription(
      "Añade una advertencia a un usuario y suma puntos de infracción.",
    )
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (target.id === interaction.user.id) {
      await interaction.reply({
        content: "🌸 No puedes advertirte a ti mismo, tontito.",
        ephemeral: true,
      });
      return;
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `Se ha aplicado una infracción a **${target.user.tag}**.`,
        )
        .addFields(
