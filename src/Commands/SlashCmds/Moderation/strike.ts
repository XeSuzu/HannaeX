...
const command: SlashCommand = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(opt => opt.setName('puntos').setDescription('Cantidad de puntos del strike').setRequired(false))
    .addStringOption(opt => opt.setName('razon').setDescription('Razón del strike').setRequired(false))
    .setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers),

  async execute(interaction, client) {
    // Es buena práctica asegurar que estamos en un servidor antes del defer
    if (!interaction.guild) {
      return interaction.reply({ content: 'Este comando debe ejecutarse en un servidor.', ephemeral: true });
    }
    ...
    try {
      ...
      if (target.id === client.user?.id) {
        return interaction.editReply({ content: '¡No puedes hacerme eso! ٩(๏̯͡๏)۶ No puedo sancionarme a mí misma.' });
      }
      if (target.id === interaction.user.id) {
        return interaction.editReply({ content: 'No puedes sancionarte a ti mismo, ¡ten más cuidado!' });
      }
      if (target.bot) {
        return interaction.editReply({ content: 'Los otros bots también tienen sentimientos, no los sanciones.' });
      }

      // Guardar strike en la base de datos
      await addStrike(interaction.guild.id, target.id, interaction.user.id, points, reason);
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Se han añadido **${points}** punto(s) a **${target.tag}**.`)
        .addFields(
      ...
      await interaction.editReply({ content: 'Lo siento mucho, ocurrió un error inesperado al procesar el strike. ＞﹏＜' });
    }
  }
};
