  ...
  data: new SlashCommandBuilder()
    ...
    .addBooleanOption((opt) =>
      opt
        .setName("clear")
        .setDescription(
          "Borrar todos los strikes (requiere permisos de moderación)",
        )
        .setRequired(false),
    ),
  ...
  async execute(interaction, client) {
    ...
    if (!interaction.guild) {
      return interaction.reply({
        content:
          "Lo siento, este comando solo funciona dentro de un servidor. (◕︿◕✿)",
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      if (shouldClear) {
        ...
        if (!member?.permissions.has(PermissionFlagsBits.ModerateMembers)) {
          return interaction.editReply({
            content:
              '¡Oh no! No tienes permiso para borrar strikes. Necesitas el permiso de "Moderar miembros".',
          });
        }

        const deletedCount = await clearStrikesForUser(
      ...
      if (strikes.length === 0) {
        embed.addFields({
          name: "Estado",
          value: "¡Qué alegría! Este usuario no tiene strikes registrados. ✨",
        });
      } else {
        ...
        if (strikes.length > 10) {
          embed.setFooter({
            text: `Mostrando 10 de ${strikes.length} entradas totales.`,
          });
        }
      }
      ...
      await interaction.editReply({
        content:
          "Hubo un problemita al consultar la base de datos. ¡Inténtalo de nuevo más tarde! ฅ^•ﻌ•^ฅ",
      });
    }
  },
};
