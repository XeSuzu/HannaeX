...
if (!process.env.GOOGLE_SEARCH_API_KEY || !process.env.SEARCH_ENGINE_ID) {
    console.error("❌ Faltan variables de entorno para la búsqueda de Google (GOOGLE_SEARCH_API_KEY, SEARCH_ENGINE_ID).");
}
...
export async function buscarEnGoogle(consulta: string): Promise<string> {
    ...
    try {
        ...
        console.error("Error en buscarEnGoogle:", error.response?.data?.error?.message || error.message);
