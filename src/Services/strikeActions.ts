 ...
 */
export async function checkAutoActions(
  ...
  if (!member || member.id === client.user?.id || member.id === guild.ownerId)
    return;
  ...
  try {
    switch (applicableAction.action) {
      case "timeout":
        ...
        const duration =
          typeof applicableAction.duration === "number"
            ? applicableAction.duration
            : null;
    ...
    if (config.modLogChannel) {
      ...
      if (logChannel) {
        const logEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `Se aplicó **${applicableAction.action.toUpperCase()}** a ${user.tag} por acumular **${totalPoints}** puntos.`,
          )
          .setTimestamp();
